...
class VectorContextStore:
    ...
    def query_batch(self, query_texts: list[str], top_k: int = 3) -> list[list[str]]:
        """Return top-k similar context snippets for a batch of queries."""
        top_k = max(1, top_k)
        self._maybe_start_chroma_init()

        if self._chroma_collection is not None and query_texts:
            try:
                result = self._chroma_collection.query(
                ...
                documents = result.get("documents", [])
                if documents:
                    return [[str(item) for item in doc_list if isinstance(item, str)] for doc_list in documents]
            except Exception as exc:
                logger.warning("Vector batch query failed, falling back to lexical similarity: %s", exc)

        return [self._lexical_query(query_text=q, top_k=top_k) for q in query_texts]
